  ...
  describe("building a link", () => {
    ...
    describe("without a ref id", () => {
      ...
      it("should throw an illegal state error.", async() => {
        ...
        await expectAsync(subject.build())
            .toBeRejectedWithError(IllegalStateError, "Required ref id was not found, ILIAS default link build failed.");
      });
    });
    ...
    describe("without an authenticated user", () => {
      ...
      it("should throw a no such element error.", async() => {
        ...
        await expectAsync(subject.build())
          .toBeRejectedWithError(NoSuchElementError, "No authenticated user found, unable to build default ILIAS link.");
      });
    });
    ...
    describe("with an authenticated user and a valid ref id", () => {
      ...
      it("should build the ILIAS default object link.", async() => {
        ...
        await expectAsync(subject.build())
          .toBeResolvedTo(`${installation}/goto.php?target=ilias_app_auth|${user.iliasUserId}|${refId}|default|${token}`);
      });
    });
  })
});
