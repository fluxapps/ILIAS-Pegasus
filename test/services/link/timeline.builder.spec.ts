  ...
  describe("building a link", () => {
    ...
    context("without a ref id", () => {
      ...
      it("should throw an illegal state error.", async () => {
        ...
        await chai.expect(subject.build())
          ...
          .to.eventually.have.property("message", "Required ref id was not found, ILIAS timeline link build failed.");
      });
    });
    ...
    context("without an authenticated user", () => {
      ...
      it("should throw a no such element error.", async () => {
        ...
        await chai.expect(subject.build())
          ...
          .to.eventually.have.property("message", "No authenticated user found, unable to build timeline ILIAS link.");
      });
    });
    ...
    context("with an authenticated user and a valid ref id", () => {
      ...
      it("should build the ILIAS timeline link.", async () => {
        ...
        await chai.expect(subject.build())
          .to.be.eventually.equal(`${installation}/goto.php?target=ilias_app_auth|${user.iliasUserId}|${refId}|timeline|${token}`);
      });
    });
  })
});
